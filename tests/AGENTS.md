...
Executable contracts for Irigate configuration, background reload, systemd user-service lifecycle, transport, routing, isolation, shutdown, qualification, reporting, payload logging, and client compatibility.
...
- `test_systemd.py` owns systemd user-unit rendering, including preservation of a tool-venv interpreter symlink, private referenced-environment mirroring, active-service restart on sync, setup enable/start, reload delegation, and nested CLI argument handling.
