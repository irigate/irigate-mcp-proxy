...
- `test_*.py` files own the behavioral contracts documented in `IMPLEMENTATION.md`, including CLI tool discovery, calls and process reports, agent attribution, selector parsing, selected-only activation, and independent shared and isolated worker idle expiry.
