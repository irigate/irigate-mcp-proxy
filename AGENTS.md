...
- **Workspace inputs fail closed.** Filesystem workspaces are reserved for required directory inputs on isolated upstreams. Configuration, canonical allowed-root authorization, namespaced HTTP parsing, immutable downstream-session binding, worker-local argument rendering, and input-keyed process isolation are implemented and documented. Shipped profiles must use narrow `allowed_roots`; no profile may default to `/**`.
