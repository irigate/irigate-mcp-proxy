...
- **Runtime process state uses XDG state storage.** An omitted `runtime_report_path` resolves to `${XDG_STATE_HOME:-~/.local/state}/irigate/<profile>/runtime-report.json`; its credential-free process-control document remains adjacent. Explicit profile paths are exact overrides.
