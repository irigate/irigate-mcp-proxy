...
- **Workspace inputs are staged and fail closed.** Filesystem workspaces are reserved for required directory inputs on isolated upstreams. Configuration, canonical allowed-root authorization, namespaced HTTP parsing, immutable downstream-session binding, worker argument rendering, and input-keyed process isolation are implemented; the benchmark profile and durable client documentation remain unfinished, so the feature must not yet be presented as usable client-facing behavior.
