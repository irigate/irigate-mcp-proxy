...
class RestartError(ValueError):
    """A safe-to-display restart control error."""
...
class RestartControl:
    ...
    def __post_init__(self) -> None:
        if self.schema_version != CONTROL_SCHEMA_VERSION:
            raise RestartError("unsupported restart control schema_version")
        if not isinstance(self.profile, str) or not self.profile:
            raise RestartError("restart control profile must be non-empty")
        if not isinstance(self.config_path, str) or not self.config_path:
            raise RestartError("restart control config_path must be non-empty")
        if not Path(self.config_path).is_absolute():
            raise RestartError("restart control config_path must be absolute")
        if not isinstance(self.pid, int) or isinstance(self.pid, bool) or self.pid <= 0:
            raise RestartError("restart control pid must be positive")
        if not isinstance(self.instance_id, str) or not self.instance_id:
            raise RestartError("restart control instance_id must be non-empty")
        if not isinstance(self.version, str) or not self.version:
            raise RestartError("restart control version must be non-empty")
    ...
    def from_dict(cls, value: Any) -> RestartControl:
        if not isinstance(value, dict) or set(value) != _CONTROL_FIELDS:
            raise RestartError("restart control document has invalid fields")
        try:
            return cls(**value)
        except TypeError as exc:
            raise RestartError("restart control document has invalid fields") from exc


def control_path(runtime_report_path: Path | None) -> Path:
    if runtime_report_path is None:
        raise RestartError("profile has no runtime_report_path; restart is unavailable")
