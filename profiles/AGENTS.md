...
- Shipped profiles omit `runtime_report_path` so they exercise the profile-scoped XDG state default.
...
- Windows-native benchmark upstreams use explicit `execution: wsl-windows` so long-running brokers refresh the WSL interop endpoint before each spawn. Windows Chrome declares `wsl_path_arguments` for its documented file and report output fields.
